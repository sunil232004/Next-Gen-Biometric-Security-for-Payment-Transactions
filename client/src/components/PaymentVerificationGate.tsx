...
import { loadFaceApiModels, extractFaceEmbedding, embeddingToArray } from '@/lib/faceApi';

// ... (keep surrounding code same) ...

type BiometricType = 'fingerprint' | 'face' | 'voice';
type VerificationStep = 'password' | 'biometric';
...
export default function PaymentVerificationGate({
  ...
  // Always show options as requested
  const showFingerprint = true;
    ...
    try {
      ...
      if (response.success) {
        ...
        toast({
          ...
          description: "Please proceed with biometric verification",
        });
      } else {
  ...
  const captureFaceAndVerify = async () => {
    ...
    try {
      ...
      // Verify face embedding with backend
      const embeddingArray = embeddingToArray(embedding);
      const response = await apiRequest('/api/v2/face/verify', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('paytm_auth_token')}`
        },
      ...
      if (response.success && response.verified) {
        ...
        toast({
          title: 'Payment Successful',
          description: 'Face verified using ML recognition.'
        });
        setTimeout(() => {
          onSuccess('face');
        }, 500);
      } else {
        throw new Error(response.message || 'Face verification failed - face does not match');
      }
    } catch (err: any) {
      setVerificationStatus('failed');
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-white/80 text-sm">
                {step === 'password' ? 'Step 1: Password Verification' : 'Step 2: Biometric Authentication'}
              </p>
            </div>
          </div>
                ...
                <div className="py-4">
                  ...
                  {selectedMethod === 'fingerprint' && (
                    <div className="text-center">
                      <div
                        className={`w-24 h-24 mx-auto mb-4 border-4 rounded-full flex items-center justify-center transition-all ${verificationStatus === 'verifying' ? 'border-blue-500 animate-pulse' :
                  ...
                  {selectedMethod === 'voice' && (
                    <div className="text-center">
                      <div
                        className={`w-24 h-24 mx-auto mb-4 border-4 rounded-full flex items-center justify-center transition-all ${isProcessing && verificationStatus !== 'verifying' ? 'border-purple-500 animate-pulse' :
